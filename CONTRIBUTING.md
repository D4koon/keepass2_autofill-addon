...
* Formatting and style are owned by ESLint. Run `npm run lint` to check and `npm run lint:fix` to apply. There is no separate Prettier step; if you use VS Code, install the recommended ESLint extension (`.vscode/extensions.json`) and it will fix on save. `.editorconfig` covers basic whitespace for other editors.
